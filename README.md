...
> **Source location** is the other editable field you need to fill in. Enter `/cloudbuild.yaml` — this is the path (relative to the repository root) to the Cloud Build configuration file that defines the build, push, and deploy steps.

### How it works

1. Cloud Build checks out your `main` branch.
2. It reads `/cloudbuild.yaml`, which orchestrates a three-step pipeline:
   - **Step 1 (build):** builds the Docker image, injecting all `VITE_*` variables as `--build-arg` so Vite bakes them into the static `dist/` bundle. Non-secret variables come from Cloud Build substitutions; `VITE_GOOGLE_API_KEY` and `VITE_GEMINI_API_KEY` are fetched securely from Google Cloud Secret Manager.
...
4. The container serves the pre-built static SPA via nginx.
